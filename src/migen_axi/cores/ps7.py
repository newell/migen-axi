...
from ..interconnect import Interface, InterconnectPointToPoint, dmac_bus, wrshim
...
sig_name = comp(
    "".join,
    operator.methodcaller("split", "_"),
    get_in([-1, 0]),
    operator.attrgetter("backtrace"),
)
...
def connect_s_axi(interface):
    return thread_first(interface, (connect_interface, False), fix_arsize, fix_awsize)


def connect_m_axi(interface):
    return thread_first(interface, (connect_interface, True), fix_arsize, fix_awsize)


axi_global_rec = partial(
    Record,
    [
        ("aclk", 1, DIR_M_TO_S),
...
def tristate(name, n=1):
    return name, [("i", n, DIR_S_TO_M), ("o", n, DIR_M_TO_S), ("t_n", n, DIR_M_TO_S)]
...
gpio_rec = partial(
    Record, [("i", 64, DIR_S_TO_M), ("o", 64, DIR_M_TO_S), ("t_n", 64, DIR_M_TO_S)]
)
...
bibuf = comp(
    apply_map(partial(Instance, "BIBUF")), dict, partial(zip, ["io_PAD", "io_IO"])
)

bufg = comp(apply_map(partial(Instance, "BUFG")), dict, partial(zip, ["i_I", "o_O"]))
...
class ENETRx(Module):
    def __init__(self, pads, gmii):
        ...
        self.sync += [
            gmii.rxd.eq(pads.rxd),
            gmii.rx_dv.eq(pads.rx_dv),
            gmii.rx_er.eq(pads.rx_er),
        ]
...
class ENETTx(Module):
    def __init__(self, pads, gmii):
        ...
        self.sync += [
            pads.txd.eq(gmii.txd),
            pads.tx_en.eq(gmii.tx_en),
            pads.tx_er.eq(gmii.tx_er),
            gmii.col.eq(pads.col),
            gmii.crs.eq(pads.crs),
        ]
...
class ENET(Module):
    def __init__(self, pads):
        ...
        self.comb += [
            ClockSignal("eth_rx").eq(pads.rx_clk),
            ClockSignal("eth_tx").eq(pads.tx_clk),
        ]
        self.submodules += [
...
class PS7(Module):
    def __init__(
        self,
        pads=SimpleNamespace(ps=None, ddr=None, enet0=None, enet1=None),
        ps_cd_sys=True,
        **kwargs
    ):
        ...
        self.specials += [
            bibuf([pads_ddr_v[i], ddr_buf_v[i]]) for i in range(len(pads_ddr_v))
        ]
        self.specials += [
        ...
        self.comb += [
            ...
            irq.f2p[16:20].eq(
                Cat(
                    self.core.core0.nirq,
                    self.core.core1.nirq,
                    self.core.core0.nfiq,
                    self.core.core1.nfiq,
                )
            ),
        ...
        ps7_attrs = pipe(
            [
                ...
                keymap(
                    str_replace("TRACE", "EMIOTRACE"), connect_interface(self.trace)
                ),
                ...
                dict(
                    io_PSPORB=ps_buf.por_b,
                    io_PSSRSTB=ps_buf.srst_b,
                    io_PSCLK=ps_buf.clk,
                ),
                keymap(
                    str_replace("ENET", "EMIOENET0"), connect_interface(self.enet0.enet)
                ),
                keymap(
                    str_replace("ENET", "EMIOENET1"), connect_interface(self.enet1.enet)
                ),
            ],
            R.apply(merge),
